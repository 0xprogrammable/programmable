...
export const V4_RELEASE_BINDING_PATH = "docs/operations/releases/custom-launch-v4.1.2/cli-release-binding.json";
const PACKAGE = Object.freeze({ name: "@programmable/launch", version: "4.1.2",
  tag: "programmable-launch-v4.1.2", repository: "programmablehq/PROGRAMMABLE" });
...
export function createV412ClientReleaseBinding({ repositoryRoot }) {
...
export function auditV412ClientSource({ repositoryRoot, bindingBytes = null }) {
  const root = realpathSync(path.resolve(repositoryRoot));
  ...
  const expected = createV412ClientReleaseBinding({ repositoryRoot: root });
...
export function auditV4ReleaseBinding(options) {
  const client = auditV412ClientSource(options);
  // The historical identity remains 4.1.0. Never substitute the 4.1.2 package
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  ...
  if (!["create-client-binding", "audit-source", "audit", "verify-release-ready"].includes(command)
    ...
    throw new Error("Usage: programmable-launch-v412-release-binding.mjs <create-client-binding|audit-source|audit|verify-release-ready> --repository-root PATH");
  }
  if (command === "create-client-binding") {
    process.stdout.write(`${JSON.stringify(createV412ClientReleaseBinding({ repositoryRoot }), null, 2)}\n`);
  } else {
    const result = command === "audit-source" ? auditV412ClientSource({ repositoryRoot })
