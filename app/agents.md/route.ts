...
export function GET() {
  ...
  const content = [
    ...
    "New combined keys grant custom-launch:create, custom-launch:read, modules:submit and modules:read. Existing keys keep their original scopes, wallet binding, expiry and chain restrictions. A scope error is not a reason to rotate or broaden a key automatically. The key cannot sign transactions, move funds, approve modules or change fee recipients.",
    "## Choose the workflow",
    `- Configure a normal coin: ${entry.website.moduleMode}. Choose the name, symbol, image, initial buy and creator swap fees, then optionally add modules. Review and sign with the connected wallet. Read ${entry.workflows.moduleLaunch.availability} for the active engine and catalog. This browser wallet path does not have a generic Module Mode API create endpoint; do not invent one.`,
