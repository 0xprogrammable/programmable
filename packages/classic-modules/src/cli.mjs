...
import { createModuleApiClient, ModuleApiError } from './open-client.mjs';
...
Commands:
  ...
  module-context --api-origin https://api.example
...
Authenticated commands read PROGRAMMABLE_API_KEY from the environment, never an argument.
PROGRAMMABLE_MODULES_API_KEY remains supported; if both are set, their values must match.
Use a key with modules:submit and modules:read. The descriptor author must be that key's EVM wallet;
module-context reads this wallet and the prerequisites before building. Default rewardWallet to that author
unless the contributor specifies another nonzero EVM payout wallet. Both are explicit immutable descriptor fields.
Source intake accepts new runtime and host capability namespaces; review adapter coverage is a separate step.
...
const fields = {
  ...
  'prepare-module-submission': ['package', 'out'], 'module-capabilities': ['api-origin'], 'module-context': ['api-origin'],
...
export async function runCli(args, { stdout = process.stdout, stderr = process.stderr, env = process.env } = {}) {
  const secrets = [env.PROGRAMMABLE_API_KEY, env.PROGRAMMABLE_MODULES_API_KEY]
    .filter((value) => typeof value === 'string' && value.length > 0);
  const apiKey = secrets[0];
  const output = (value) => {
    const serialized = JSON.stringify(value, (_key, field) => typeof field === 'string'
      ? secrets.reduce((text, secret) => text.split(secret).join('[redacted]'), field) : field, 2);
    return `${serialized}\n`;
  };
  try {
    ...
    if (['module-context', 'submit-module', 'status-module', 'review-status-module', 'list-module-submissions'].includes(command)
      && new Set(secrets).size > 1) throw new ModuleApiError('MODULE_API_KEY_CONFLICT',
      'PROGRAMMABLE_API_KEY and PROGRAMMABLE_MODULES_API_KEY identify different connections. Keep one, or set both to the same key');
    ...
    if (['prepare-module-submission', 'submit-module'].includes(command)) {
    ...
    } else if (['module-capabilities', 'module-context', 'review-capabilities', 'status-module', 'review-status-module', 'list-module-submissions'].includes(command)) {
      ...
      result = { ok: true, ...await (command === 'module-capabilities' ? client.capabilities() : command === 'module-context' ? client.context() : command === 'review-capabilities' ? client.reviewCapabilities()
