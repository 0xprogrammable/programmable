...
export function moduleReviewAdminFixture() {
  ...
  const plan: ReviewPlan = { schemaVersion: "programmable.modules.native-build-plan.v1", submissionId: subject.submissionId, requestDigest: subject.requestDigest,
    programComponentId: "program", factoryComponentId: "factory", callbackGas: 75000,
    cases: [{ id: "basic", parameters: { capNative: "1" }, budgetWei: "0", expectedDeployment: "success" }] };
  ...
  const configHash = keccak256("0x01");
  const contents: Omit<ReviewBuildArtifact, "artifactDigest"> = {
    ...
    factory: compiled("factory", "0x6001"), program: compiled("program", "0x6002"), callbackGas: plan.callbackGas,
    cases: [{ ...plan.cases[0], configBytes: "0x01", configHash, abiParameters: [] }],
  ...
  const definition: ModuleModeCatalogDefinition = { id: "synthetic-opening-cap-v1", title: "Opening buy limit", summary: "Synthetic opening limit.", detail: "A parser fixture only.", version: "1.0.0",
    engine: NATIVE_ENGINE_PROFILE, source: { path: "src/Program.sol", sha256: files[1].sha256 }, schema, defaults: { capNative: "1" }, fields: { capNative: { suffix: "ETH", decimals: 18 } }, programAbi: [{ path: ["capNative"], type: "uint128" }], management: referenceManagementManifest("cap"), requiresHost: source.descriptor.requiresHost };
