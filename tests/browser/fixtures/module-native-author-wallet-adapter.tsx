...
import { revalidateModuleNativeTransaction, type PreparedModuleNativeManagement } from "../../../lib/module-mode/native-client";
...
function fixtureWallet(mode: string, count: () => void) {
  return { wallet: { account: actor, chainId: "4663" }, authenticated: true, sessionReady: true, openWallet: () => undefined, switchNetwork: async () => undefined,
    sendModuleModeTransaction: async (prepared: PreparedModuleNativeManagement) => {
      await revalidateModuleNativeTransaction(prepared, actor);
      ...
      return f.mined(prepared).transactionHash;
    } };
}
...
export function FixtureWallet({ children }: { children: ReactNode }) {
  ...
  const wallet = fixtureWallet(mode, () => { const next = Number(localStorage.getItem("fixture:author-sends") ?? 0) + 1; localStorage.setItem("fixture:author-sends", String(next)); setCount(next); });
  return <context.Provider value={wallet}><aside className="fixture-banner"><strong>LOCAL AUTHOR WALLET TEST</strong><p>Real Native coin controls, private validation and recovery storage with synthetic RPC responses. No live wallet or publication.</p><label>Test wallet response <select value={mode} onChange={event => setMode(event.target.value)}><option value="confirm">Simulate mined</option><option value="reject">Reject request</option><option value="uncertain">Uncertain request</option></select></label><output aria-label="Test wallet calls">Wallet calls: {count}</output></aside>{children}</context.Provider>;
}
