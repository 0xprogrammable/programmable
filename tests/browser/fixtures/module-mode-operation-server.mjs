...
export async function createModuleModeOperationServer() {
  ...
  const bundle = await build({ stdin: { contents: `import React from 'react'; import {createRoot} from 'react-dom/client'; import {ModuleModeLaunchHost} from './components/module-mode-launch-host'; import {ModuleCoinConsole} from './components/module-coin-console'; import {FixtureWallet,token} from './tests/browser/fixtures/module-mode-operation-wallet'; import './app/globals.css'; import './app/interface.css'; import './app/programmable-experience.css'; import './app/webde-final-ui.css'; createRoot(document.getElementById('root')).render(<FixtureWallet>{window.location.pathname.includes('/manage/') ? <ModuleCoinConsole token={token} releaseDigest={new URLSearchParams(window.location.search).get("releaseDigest") ?? undefined}/> : <ModuleModeLaunchHost/>}</FixtureWallet>);`, loader: "tsx", resolveDir: root },
