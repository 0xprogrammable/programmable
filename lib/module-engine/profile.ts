...
export const MODULE_ENGINE_ANY_QUOTE_ETH_SOURCE_VERSION = "module-engine-any-quote-eth-v1" as const;
export const MODULE_ENGINE_ANY_QUOTE_ETH_SOURCE_ID = keccak256(toHex("programmable.module-engine.any-quote.native-eth.v1"));
export const MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE = "robinhood-any-quote.shared-hook.native-eth.v1" as const;
export const MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE_ID = keccak256(toHex(MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE));
export const MODULE_ENGINE_ANY_QUOTE_ETH_ECONOMICS_POLICY_ID = keccak256(toHex("programmable.any-quote.base-30.creator-0-1000.native-eth.v1"));
...
export interface ModuleEngineAnyQuoteEthReleaseProfile extends Omit<ModuleEngineAnyQuoteReleaseProfile, "sourceVersion" | "engineProfile"> {
  sourceVersion: typeof MODULE_ENGINE_ANY_QUOTE_ETH_SOURCE_VERSION;
  engineProfile: typeof MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE;
}
export type ModuleEngineSharedQuoteReleaseProfile = ModuleEngineAnyQuoteReleaseProfile | ModuleEngineAnyQuoteEthReleaseProfile;
export type ModuleEngineReleaseProfile = ModuleEngineNativeReleaseProfile | ModuleEngineSharedQuoteReleaseProfile;
...
export function isModuleEngineAnyQuoteEthRelease(value: unknown): value is ModuleEngineAnyQuoteEthReleaseProfile {
  return matches(value, MODULE_ENGINE_ANY_QUOTE_ETH_SOURCE_VERSION, MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE, MODULE_ENGINE_ANY_QUOTE_ETH_ECONOMICS_POLICY_ID);
}
/** Pool/LP capability only. Fee denomination always uses the exact source discriminator. */
export function isModuleEngineSharedQuoteRelease(value: unknown): value is ModuleEngineSharedQuoteReleaseProfile {
  return isModuleEngineAnyQuoteRelease(value) || isModuleEngineAnyQuoteEthRelease(value);
}
export function moduleEngineSharedQuoteProfileId(value: ModuleEngineSharedQuoteReleaseProfile): Hex {
  return isModuleEngineAnyQuoteEthRelease(value) ? MODULE_ENGINE_ANY_QUOTE_ETH_PROFILE_ID : MODULE_ENGINE_ANY_QUOTE_PROFILE_ID;
}
export function moduleEngineSourceId(value: unknown): Hex {
  ...
  if (isModuleEngineAnyQuoteEthRelease(value)) return MODULE_ENGINE_ANY_QUOTE_ETH_SOURCE_ID;
...
export function moduleEngineContractRoles(value: unknown) {
  ...
  return isModuleEngineSharedQuoteRelease(value) ? MODULE_ENGINE_ANY_QUOTE_CONTRACTS : MODULE_ENGINE_CONTRACTS;
}
