...
async function selection(input: Selection, deps: AnyQuoteIntegrationDependencies) {
  ...
  if (release.releaseDigest !== digest || !isModuleEngineAnyQuoteRelease(release)) throw new AnyQuoteErrorV1("MODULE_UNAVAILABLE");
...
export async function readAnyQuoteReadiness(input: Selection & { quoteAsset: string }, deps: AnyQuoteIntegrationDependencies = {}) {
  ...
  return (deps.readiness ?? assessAnyQuoteAssetV1)({ quoteAsset: input.quoteAsset }, anyQuotePreparationOptionsV1(deps));
}
