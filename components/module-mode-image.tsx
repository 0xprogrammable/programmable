      ...
      <div className={styles.imagePicker}>
        ...
        <div className={styles.imagePickerContent}>
          ...
          <div className={styles.imagePickerActions}>
            <button type="button" className={styles.secondaryButton} hidden={compact} disabled={preparing} aria-busy={preparing} data-invalid={Boolean(message)} aria-describedby={`module-token-image-help${preparing ? " module-token-image-status" : ""}${message ? " module-token-image-error" : ""}`} onClick={() => input.current?.click()}><Upload size={16} aria-hidden="true" />{image.kind === "local" ? "Change image" : "Choose image"}</button>
