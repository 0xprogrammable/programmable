...
export function ModuleEngineConsole({ release, template, token, client: suppliedClient, wallet, onConnect, onSwitch, onSubmit, blocked, blockedReason, statusContent }: ModuleEngineConsoleProps) {
  ...
  const anyQuote = isModuleEngineAnyQuoteRelease(release) && profile === "quote-shared-v1";
  ...
  return <section className={`${styles.page} ${engineStyles.root}`} aria-labelledby="engine-console-title">
    <header className={styles.heading}><div className={styles.titleRow}><h1 id="engine-console-title">{template.manifest.manifest.catalogDefinition.title}</h1></div><p>{anyQuote ? "Buy and sell with ETH. Claim rewards in the pool pair token." : "Manage this coin and claim the ETH fees owed to your wallet."}</p><code className={engineStyles.inlineCode}>{token}</code></header>
    {statusContent}
    ...
    {current ? <div className={engineStyles.stack} hidden={Boolean(prepared)}>
      ...
      <section className={styles.formPanel}><h2>{anyQuote ? "Pool pair rewards" : "ETH fee claims"}</h2><dl className={styles.reviewRows}><div><dt>Claimable by your wallet</dt><dd>{formatUnits(current.fees.claimable, anyQuote ? current.quoteDecimals : 18)} {anyQuote ? "pool pair tokens" : "ETH"}</dd></div><div><dt>Already claimed</dt><dd>{formatUnits(current.fees.claimed, anyQuote ? current.quoteDecimals : 18)} {anyQuote ? "pool pair tokens" : "ETH"}</dd></div><div><dt>Accrued from this launch</dt><dd>{formatUnits(current.fees.contributionByLaunch, anyQuote ? current.quoteDecimals : 18)} {anyQuote ? "pool pair tokens" : "ETH"}</dd></div>{anyQuote ? <div><dt>Reward token</dt><dd>{current.launch.quoteAsset}</dd></div> : null}</dl><p className={styles.help}>{anyQuote ? "The full 0.3% module fee accrues to its designated recipient. Separate creator fees accrue to the creator recipients. Claiming sends your balance of this pool pair token to your wallet." : "Creator, author and platform allocations accrue to their entitled wallets. This claim withdraws your wallet’s total fee balance in this ledger to your wallet."}</p><div className={engineStyles.actions}><button id="engine-claim-review" type="button" className={styles.secondaryButton} disabled={busy || blocked || current.fees.claimable === 0n || Boolean(prepared)} onClick={() => void prepareClaim()}>{anyQuote ? "Review reward claim" : "Review ETH claim"}</button></div></section>
