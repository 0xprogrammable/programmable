...
export function ModuleEngineConsole({ release, template, token, client: suppliedClient, wallet, onConnect, onSwitch, onSubmit, blocked, blockedReason, statusContent }: ModuleEngineConsoleProps) {
  ...
  function backToEdit() { const id = prepared?.kind === "approve" ? "engine-operation-approval-review" : prepared?.kind === "claim" ? "engine-claim-review" : "engine-action-review"; setPrepared(null); requestAnimationFrame(() => document.getElementById(id)?.focus()); }
  ...
  async function prepareAction() {
    ...
    try {
      ...
      if (!["fulfill", "refund"].includes(selected) && BigInt(parseExactUnits(amount, selected === "sell" ? 18 : current.quoteDecimals)) === 0n) throw new Error("Enter an amount greater than zero.");
      let result;
  ...
  return <section className={`${styles.page} ${engineStyles.root}`} aria-labelledby="engine-console-title">
    <header className={styles.heading}><div className={styles.titleRow}><h1 id="engine-console-title">{template.manifest.manifest.catalogDefinition.title}</h1></div><p>Manage this coin and claim the ETH fees owed to your wallet.</p><code className={engineStyles.inlineCode}>{token}</code></header>
    {statusContent}
