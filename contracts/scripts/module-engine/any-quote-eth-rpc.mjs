...
import { ANY_QUOTE_ETH_NEW_ROLES, ANY_QUOTE_ETH_REUSED_ROLES, assertAnyQuoteEthProfile } from './any-quote-eth-core.mjs';

export async function observeAnyQuoteEthBindings(plan, providers, blockNumber, blockHash, deployedRoles = []) {
  assertAnyQuoteEthProfile(plan); r.requirePair(providers);
  need(deployedRoles.every(role => ANY_QUOTE_ETH_NEW_ROLES.includes(role)), 'Unknown deployed Any Quote role');
  ...
  for (const role of ANY_QUOTE_ETH_REUSED_ROLES) { await r.readCode(providers, p[role], block); reusedContracts[role] = plan.identityCandidate.contracts[role]; }
  ...
  if (deployedRoles.includes('host')) {
    need(['sharedHook', 'ledger'].every(role => deployedRoles.includes(role)), 'Predeployed hook and ledger observation required');
    ...
    await getter('host', 'sharedHookCodeHash', 'bytes32', p.sharedHook.runtimeCodeHash);
  }
  if (deployedRoles.includes('sharedHook')) {
    need(deployedRoles.includes('ledger'), 'Hook child ledger observation required');
    ...
    for (const [role, fields] of Object.entries(links)) if (deployedRoles.includes(role))
      for (const [name, expected] of Object.entries(fields)) await getter(role, name, 'address', expected);
    await getter('sharedHook', 'NATIVE_FEE_MAX_LOSS_BPS', 'uint256', plan.economics.nativeFeeMaxLossBps.toString());
  ...
  await at();
  return { schemaVersion: 'programmable.module-engine-any-quote-eth-deployment-bindings.v1', chainId: 4663,
    ...
    hookCreate2: { deployer: plan.steps[0].to, salt: plan.steps[0].salt, initcodeHash: plan.steps[0].initcodeHash, target: p.sharedHook.address },
    previousReleaseDigest: plan.basis.previousRelease.releaseDigest, previousGuardReleaseDigest: plan.basis.guardRelease.releaseDigest, reusedContracts, deployedContracts, reads,
...
export async function observeAnyQuoteEthStage(plan, stepIndex, providers) {
  assertAnyQuoteEthProfile(plan); r.requirePair(providers); const step = plan.steps[stepIndex]; need(step, 'Unknown Any Quote stage');
  const block = await r.commonBlock(providers), completed = plan.steps.slice(0, stepIndex).flatMap(s => s.expectedRoles);
  const engineBindings = await observeAnyQuoteEthBindings(plan, providers, block.number, block.hash, completed);
  if (await r.readCode(providers, plan.contracts[step.role], block.number, true)) {
    const observed = await observeAnyQuoteEthBindings(plan, providers, block.number, block.hash, [...completed, ...step.expectedRoles]);
...
export async function observeAnyQuoteEthReceipt(plan, entry, providers) {
  assertAnyQuoteEthProfile(plan); const result = await observeReceipt(plan, entry, providers);
  if (result.status !== 'included-code-verified-unfinalized') return result;
  return { ...result, engineBindings: await observeAnyQuoteEthBindings(plan, providers, result.receipt.blockNumber, result.receipt.blockHash,
