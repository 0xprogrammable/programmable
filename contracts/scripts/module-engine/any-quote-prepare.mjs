...
import { anyQuoteBasis, buildAnyQuotePlan, ANY_QUOTE_NEW_ROLES } from './any-quote-core.mjs';
import { anyQuoteExplorerRequests, anyQuoteSourceRequests } from './any-quote-evidence.mjs';

export async function prepareAnyQuote({ root = REPOSITORY_ROOT, parametersFile, outputDirectory, candidate = false }) {
  need(parametersFile && outputDirectory, 'Explicit owner nonce parameters and output directory required');
  const build = await sealAnyQuoteBuild({ root, candidate }), basis = await anyQuoteBasis(root);
  const plan = await buildAnyQuotePlan(build, exactJson(await readFile(parametersFile), 'Any Quote deployment parameters'), basis);
  await mkdir(outputDirectory, { recursive: true });
  const files = { 'plan.json': plan, 'build.json': build, 'basis.json': basis,
    'abi.json': { schemaVersion: 'programmable.module-engine-any-quote-deployment-abi.v1', sourceCommit: build.sourceCommit, buildDigest: build.buildDigest,
      contracts: Object.fromEntries(ANY_QUOTE_NEW_ROLES.map(role => [role, { ...plan.identityCandidate.contracts[role], abi: build.artifacts[role].abi }])) },
    'sourcify-verification-requests.json': anyQuoteSourceRequests(plan, build), 'source-verification-requests.json': anyQuoteExplorerRequests(plan, build),
  ...
  return { status: 'unsigned-unapproved-any-quote-preparation', sourceCommit: plan.sourceCommit, sourceClean: plan.sourceClean,
    ...
    sizes: Object.fromEntries(ANY_QUOTE_NEW_ROLES.map(role => [role, { runtimeBytes: plan.contracts[role].runtimeBytes,
...
async function main(argv) {
  ...
  console.log(JSON.stringify(await prepareAnyQuote(options), null, 2));
}
