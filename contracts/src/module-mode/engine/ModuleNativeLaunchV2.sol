...
import { ModuleNativeSwapRouterFactoryV2 } from "./ModuleNativeSwapRouterFactoryV2.sol";
...
///      immutable V2 engine reuses the finite-range native/token AMM curve in ClassicModulePositionPlannerV1; it has no
...
contract ModuleNativeLaunchV2 is IProgrammableClassicLaunchV1, ReentrancyGuardTransient {
    using SafeCast for *;
    ...
    ModuleNativeHookV2 public immutable feeHook;
    ModuleNativeSwapRouterV2 public immutable swapRouter;
    ModuleNativeSwapRouterFactoryV2 public immutable swapRouterFactory;
    ...
    constructor(
        ...
        ModuleNativeSwapRouterFactoryV2 swapRouterFactory_,
        ...
        if (
            feeHook_.ECONOMICS_POLICY_ID() != ECONOMICS_POLICY_ID
        ...
        if (feeHook_.ledger().ECONOMICS_POLICY_ID() != ECONOMICS_POLICY_ID) {
            revert InvalidDependency(address(feeHook_.ledger()));
        }
        ...
        ModuleNativeSwapRouterV2 router = swapRouterFactory_.create(poolManager_, feeHook_);
        if (
        ...
        returns (LaunchRecord memory result)
    {
        ...
        if (parameters.expectedRecipeHash == bytes32(0)) revert MissingExpectedRecipe();
        if (result.recipeHash != parameters.expectedRecipeHash) {
            revert RecipeHashMismatch(result.recipeHash, parameters.expectedRecipeHash);
        }
    ...
    function _deployOrReusePositionRecipient(address token, address launchWallet) private returns (address recipient) {
        bytes32 salt =
            keccak256(abi.encode("programmable.module-mode.native-position.v2", block.chainid, address(this), token));
    ...
    function _economicsHash(
        ...
        return keccak256(
            abi.encode(
                ECONOMICS_POLICY_ID,
                feeHook.ledger().configurationHash(result.poolId),
                result.hook,
    ...
    function _effectiveGraffiti(address launchWallet, bytes32 creatorSalt) private view returns (bytes32) {
        return keccak256(
            abi.encode(
                "programmable.module-mode.native-token.v2", block.chainid, address(this), launchWallet, creatorSalt
            )
        );
    }
