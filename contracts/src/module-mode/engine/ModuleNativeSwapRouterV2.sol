...
contract ModuleNativeSwapRouterV2 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    ModuleNativeHookV2 public immutable hook;
    ...
    constructor(IPoolManager poolManager_, ModuleNativeHookV2 hook_, address source_) {
        if (
            address(poolManager_).code.length == 0 || address(hook_).code.length == 0 || source_ == address(0)
                ...
                || hook_.ECONOMICS_POLICY_ID() != ECONOMICS_POLICY_ID
