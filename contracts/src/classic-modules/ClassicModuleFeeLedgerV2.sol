...
import { IClassicModuleAuthorRegistry } from "./ClassicModuleFeeLedgerV1.sol";
import { ModuleNativeEconomicsV2 as Economics } from "../module-mode/engine/ModuleNativeEconomicsV2.sol";

/// @title ClassicModuleFeeLedgerV2
/// @notice Accounts for native Module Mode fees collected under the immutable 10/0 or 10/20 policy.
/// @dev Reuses the V1 backed-claims, creator shares, wallet rotation and CTO model. Plain pools allocate every
///      platform fee unit to treasury. Pools with eligible families allocate floor(lifetime platform / 3) to
///      treasury and floor(lifetime platform * 2 / 3 / family count) equally to each family. Unallocated units
///      remain backed dust; no recipient gets a remainder bonus. Wallet changes affect future whole-unit credits.
///      Historical V1 ledgers, pools and rights are unchanged. There is no sweep, fee setter, pause or upgrade.
contract ClassicModuleFeeLedgerV2 is IUnlockCallback, ReentrancyGuardTransient {
    ...
    error InvalidNativeAccrual(uint256 received, uint256 expected);
    ...
    function accrue(bytes32 poolId, uint256 platformFee, uint256 creatorFee) external onlyHook nonReentrant {
        _accrue(poolId, platformFee, creatorFee);
    }

    /// @notice An admitted engine may instead accrue actual ETH, including an atomic converted fee receipt.
    /// @dev No credit may be backed by a quote-token IOU; the immutable engine supplies the exact native amount.
    function accrueNative(bytes32 poolId, uint256 platformFee, uint256 creatorFee)
        external
        payable
        onlyHook
        nonReentrant
    {
        if (msg.value != platformFee + creatorFee) {
            revert InvalidNativeAccrual(msg.value, platformFee + creatorFee);
        }
        _accrue(poolId, platformFee, creatorFee);
    }

    function _accrue(bytes32 poolId, uint256 platformFee, uint256 creatorFee) private {
    ...
    function platformFeeBps(bytes32 poolId) external view returns (uint16) {
        _requireRegistered(poolId);
        return Economics.platformFeeBps(_moduleFamilies[poolId].length);
    }
    ...
    function _allocatePlatform(bytes32 poolId, uint256 previousPlatform, PoolAccounting storage accounting) private {
        ...
        if (count == 0) {
            uint256 delta = accounting.platformReceived - previousPlatform;
            accounting.treasuryCredited += delta;
            _credit(poolId, treasury, delta);
            return;
        }
        uint256 treasuryDelta = accounting.platformReceived / 3 - previousPlatform / 3;
        accounting.treasuryCredited += treasuryDelta;
        _credit(poolId, treasury, treasuryDelta);

        uint256 previousAuthors = FullMath.mulDiv(previousPlatform, 2, 3);
        uint256 nextAuthors = FullMath.mulDiv(accounting.platformReceived, 2, 3);
        uint256 familyDelta = nextAuthors / count - previousAuthors / count;
