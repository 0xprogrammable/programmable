...
interface IAnyQuoteEthUniversalRouterV1 {
    function execute(bytes calldata commands, bytes[] calldata inputs, uint256 deadline) external payable;
}

interface IAnyQuoteEthHookManagerV1 {
    function poolManager() external view returns (IPoolManager);
}

/// @notice Any Quote launches with immediate native ETH fees and the existing permanently locked LP engine.
/// @dev Host swaps are optional. The shared hook converts fees on every pool swap, including external V4 routers.
contract ModuleEngineAnyQuoteEthHostV1 is ReentrancyGuardTransient, IModuleEngineAdmissionV1 {
    ...
    bytes32 public constant SOURCE_VERSION = keccak256("programmable.module-engine.any-quote.native-eth.v1");
    ...
    IAnyQuoteEthSharedHookV1 public immutable sharedHook;
    bytes32 public immutable sharedHookCodeHash;
    ...
    bytes32 public constant PROFILE_ID = keccak256("robinhood-any-quote.shared-hook.native-eth.v1");
    ...
    constructor(
        ...
        IAnyQuoteEthSharedHookV1 sharedHook_,
        bytes32 sharedHookCodeHash_,
        ...
        registry = registry_;
        // The hook is deployed first with this host's nonce-predicted address. Its source/runtime
        // is independently bound by the release; accepting an arbitrary hook is not a catalog admission.
        if (
            address(sharedHook_).code.length == 0 || address(sharedHook_).codehash != sharedHookCodeHash_
                || sharedHook_.host() != address(this)
                || address(IAnyQuoteEthHookManagerV1(address(sharedHook_)).poolManager()) != address(poolManager_)
        ) revert InvalidQuoteInfrastructure();
        sharedHook = sharedHook_;
        sharedHookCodeHash = sharedHookCodeHash_;
        AnyQuoteEthLedgerV1 ledger_ = AnyQuoteEthLedgerV1(sharedHook_.ledger());
        if (
            address(ledger_).code.length == 0 || ledger_.host() != address(this)
                || ledger_.hook() != address(sharedHook_) || address(ledger_.poolManager()) != address(poolManager_)
                || ledger_.rewardAdmin() != rewardAdmin_ || ledger_.ECONOMICS_POLICY_ID() != ECONOMICS_POLICY_ID
                || ledger_.treasury() != A.PLATFORM_RECIPIENT
        ) revert InvalidQuoteInfrastructure();
        ledger = ledger_;
    }
